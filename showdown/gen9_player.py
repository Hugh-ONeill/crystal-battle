...
# ---- lead prior (2026-08-07) ------------------------------------------
# Nudges the preview lead ranking with what we have MEASURED about leading
# each species, sourced from showdown/lead_measured.json (generated by
# build_lead_measured.py from within-matchup matched deltas).
#
# WHY POLICY-SHAPED AND NOT AN EVAL TERM: this steers one decision rather
# than scoring a state, and the campaign record is that policy injections
# convert where value injections do not (the monotype policy net worked and
# its value net did not; the LLM overlay's surviving channel is the reply
# DISTRIBUTION). It also lands on a decision the search structurally cannot
# get right — the cost of spending a late-game wincon as a turn-1 scout is
# ~30 turns beyond any preview horizon.
#
# ONLY SIGNIFICANT ENTRIES ARE USED. On 2026-08-07 a Ting-Lu "pilot gap" was
...
def _lead_prior_table() -> dict:
    """species -> measured winrate delta (as a fraction), significant only."""
    global _LEAD_PRIOR_CACHE
    if _LEAD_PRIOR_CACHE is None:
        table = {}
        try:
            import json as _json
            raw = _json.loads(
                (Path(__file__).parent / "lead_measured.json").read_text())
            for sp, v in (raw.get("leads") or {}).items():
                lo, hi = v.get("ci95_pp", [0, 0])
                if lo > 0 or hi < 0:          # CI excludes zero
                    table[_normalize(sp)] = v["delta_pp"] / 100.0
        except Exception:
            table = {}
        _LEAD_PRIOR_CACHE = table
    return _LEAD_PRIOR_CACHE


def _apply_lead_prior(vals, species, scale):
    """Add scale x measured-delta to each candidate lead's row value.

    `vals` are position values in winrate-like units and the preview matrix
    is measured-FLAT (row-min spreads 0.01-0.12), so a full-strength prior
    would dominate rather than nudge — hence the scale, and hence shipping
    it off by default until an A/B says otherwise.
    """
    table = _lead_prior_table()
    if not table or not scale:
        return vals, []
    out = list(vals)
    applied = []
    for i, sp in enumerate(species):
        d = table.get(_normalize(sp))
        if d:
            out[i] += scale * d
            applied.append(f"{_normalize(sp)}{scale * d:+.3f}")
    return out, applied
    ...
    async def teampreview(self, battle):
        ...
        try:
            ...
            if self._preview_lead_ev and self._opp_profile:
                ...
                vals = [min(row) for row in matrix]
                pool = _lead_pool(matrix)
            # LEAD PRIOR (CB_LEAD_PRIOR, default off): nudge the ranking by
            # what we have MEASURED about leading each species. Applied to
            # `vals` BEFORE the pool and the argmax so it steers both the
            ...
            if _lp:
                ...
                # poke-env's team dict is not guaranteed to share that order.
                # A misalignment here would silently apply each prior to the
                # wrong row — the worst kind of bug, since it still "works".
                from monotype.lead_picker import split_team_body as _stb
                our_species = [b.split("@")[0].split("\n")[0].strip()
                               for b in _stb(self._team_paste)]
                vals, applied = _apply_lead_prior(vals, our_species, _lp)
                if applied:
                    pool = _near_tie_pool(vals)
                    lead_idx = max(range(len(vals)), key=lambda i: vals[i])
                    if self._verbose:
